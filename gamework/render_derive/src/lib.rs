// Based on: http://nercury.github.io/rust/opengl/tutorial/2018/06/27/opengl-in-rust-from-scratch-09-vertex-attribute-format.html
#![recursion_limit = "128"]
...
fn generate_impl(ast: &syn::DeriveInput) -> quote::Tokens {
    ...
    quote! {
        impl #ident #generics #where_clause {
            ...
            pub fn vertex_attrib_pointers(gl: &glow::Context) {
...
fn generate_struct_field_vertex_attrib_pointer_call(field: &syn::Field) -> quote::Tokens {
    ...
    let location_value: u32 = match location_attr.value {
    ...
    quote! {
        ...
        let offset = offset + ::std::mem::size_of::<#field_ty>() as i32;
    }
}
